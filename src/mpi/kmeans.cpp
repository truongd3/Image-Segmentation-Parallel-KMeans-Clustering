...
void Kmeans::fit(const std::vector<cv::Vec3f>& points,
    ...
    centers = std::vector<cv::Vec3f>(k);
    if (rank == 0) {
        std::mt19937 rng(12345);
        std::uniform_int_distribution<int> dist(0, N - 1);
        for (int j = 0; j < k; ++j)
            centers[j] = points[dist(rng)];
    }

    // Broadcast initial centers
    MPI_Bcast(centers.data(), k * 3, MPI_FLOAT, 0, MPI_COMM_WORLD);

    std::vector<cv::Vec3f> new_centers(k);
    std::vector<cv::Vec3f> local_sums(k);
    std::vector<cv::Vec3f> global_sums(k);
    ...
    for (int iter = 0; iter < max_iters; iter++) {
        ...
        MPI_Allreduce(local_sums.data(), global_sums.data(), k * 3, MPI_FLOAT,
                      MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(local_counts.data(), global_counts.data(), k, MPI_INT,
                      MPI_SUM, MPI_COMM_WORLD);
        ...
        for (int j = 0; j < k; ++j) {
            if (global_counts[j] > 0)
                new_centers[j] = global_sums[j] * (1.0f / global_counts[j]);
            else
    ...
    MPI_Allreduce(labels.data(), global_labels.data(), N, MPI_INT, MPI_MAX,
                  MPI_COMM_WORLD);
